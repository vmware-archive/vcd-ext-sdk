...
of using the [vCD Extensibility SDK][vcd-ext-sdk]{:target="_blank"} to develop a fully working vCloud Director Plugin.
...
This Ticketing example was created and tested with:

- npm - `v6.14.4`
- node - `v12.16.3`
- vcd-ext - `v0.0.13-beta.1`
- VMware Cloud Director test cell - `v10.3.1`
...
- [Clarity Design System][clarity-design]{:target="_blank"}
- [VCD UI Common Components][ui-components]{:target="_blank"}

## The full ticketing example

The complete Ticketing solution example can be found in the [Ticketing example repository][ticketing-example-repo]{:target="_blank"}.
Forking and cloning the repository locally will allow you to run the full example. 
If you are already familiar with the [vCD Extensibility SDK][vcd-ext-sdk]{:target="_blank"} and would like to simply experiment with the 
Ticketing example, you are welcome to skip the step-by-step tutorial.

### Running the ticketing example
The project uses monorepo structure to hold both UI and Defined Entities projects supported by `lerna`.

Inside `ticketing` root folder:

- `npm install` - installs all monorepo npm packages 
- `npm run bootstrap` - will run `npm install` for each project and will also create symbolic links (symlinks) 
between projects referencing each other.
- `npm run build` - will run build for each project.
- `vcd-ext login userTypeAlias https://vcd-cell.com/cloudapi sysAdminName` - use to login as system administrator to
the VCD test cell
- `npm run deploy` - will deploy each plugin in the solution (UI and Defined Entities) to the previously authenticated cell
- `npm run start` - will start an Angular application emulator wrapping the UI plugin, hosted in a local http server

### Bypassing the CORS HTTP error cause by the local http server
There are certain API calls which use absolute VCD cell url and are not intercepted by the local http proxy usually configured
to avoid this issue. This results in the following issue in the console when creating a ticket:

<span style="color:#C21D00">Access to XMLHttpRequest at 'https://vcd-cell.com/api/task/de358cfa-ade5-4e48-a914-0d5f213c0cbb' from origin 
'http://localhost:4200' has been blocked by CORS policy: Response to preflight request doesn't pass access control 
check: No 'Access-Control-Allow-Origin' header is present on the requested resource<span>
 
There are two ways to fix that:

- SSH to the VCD Cell and enable CORS by configuring the HTTP server to add `Access-Control-Allow-Origin: *` to all response headers
<br/>
OR
- Open your browser with disabled security - here's how to open Chrome with disabled security: `open -n -a /Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome --args --user-data-dir="/tmp/chrome_dev_test" --disable-web-security`
...
`npm i -g @vcd/ext-cli@0.0.13-beta.1`
...
`? Your solution name` Type in `ticketing`
...
`? Specify minimum required Cloud Director version` Press enter to skip
...
`? Select extensibility elements (Press <space> to select, <a> to toggle all, <i> to invert selection)`<br />
 `❯◯ Import vApp` <br />
 `◯ Defined Entities` <br />
 `◯ UI Plugin`
...
[Ticketing example repository][ticketing-example-repo]{:target="_blank"} containing the complete implementation. 
...
In the UI implementation we will be using [Clarity Design System][clarity-design]{:target="_blank"} and [VCD UI Common Components][ui-components]{:target="_blank"} which we 
...
over the `ApiClientService`. For implementation details please refer to [Ticketing example repository][ticketing-example-repo]{:target="_blank"}.
...
The [@vcd][npm-vcd-scope]{:target="_blank"} scope contains a collection of publicly available packages, created from different components used in 
[VMware Cloud Director's][vcd]{:target="_blank"} UI. Therefore, we can take advantage of them when creating our plugin, to get as close as possible 
...
Before we continue with the UI implementation, it is important to understand the significance of using the [VCD UI Common Components][ui-components]{:target="_blank"} library.
One of the main goals of this library is to allow plugin developers to create plugins with VCD Unified UX. This is why, 
it is important to always try using existing UI Components and utilities from [VCD UI Common Components][ui-components]{:target="_blank"}, before
...
For its implementation we will be using [Clarity Modal][clarity-modal]{:target="_blank"}. 
At the end, the delete modal will look like this
![Initial UI]({{ site.baseurl }}/assets/images/delete-confirmation-modal.png)

For further implementation details please refer to the [Ticketing example repository][ticketing-example-repo]{:target="_blank"}.
...
For the implementation we will be using [Reactive Forms][reactive-forms]{:target="_blank"} together with some components from `@vcd/ui-componets`.
...
Note the combination between Clarity components and vCD Common Components, for instance [clr-modal][clarity-modal]{:target="_blank"}, 
[clr-textarea-container][clarity-text-area]{:target="_blank"} and [vcd-form-select][ui-component-vcd-select]{:target="_blank"}. The modal in **Create** mode looks like this 

![Ticketing overview]({{ site.baseurl }}/assets/images/new-ticket-modal.png)

##### The typescript file
For the typescript implementation in `ticket-modal.component.ts` please refer to [Ticketing example repository][ticketing-example-repo]{:target="_blank"}, 
since there isn't anything particularly specific to discuss there.

#### Ticketing main page
The main page will mainly consist of [VCD UI Common Components][ui-components]{:target="_blank"}.
...
For the main page we are using the [vcd-datagrid][ui-component-vcd-grid]{:target="_blank"} and [vcd-error-banner][ui-component-vcd-error-banner]{:target="_blank"}
...
##### The typescript file
For implementation details in the `ticketing.component.ts` file, please refer to [Ticketing example repository][ticketing-example-repo]{:target="_blank"}. 
For further understanding of the `vcd-grid` configuration refer to [vCD UI Components Datagrid][ui-component-vcd-grid]{:target="_blank"}. 
...
#### Running the example
Once you are done with the implementation, please refer to [Running the ticketing example](#running-the-ticketing-example)
