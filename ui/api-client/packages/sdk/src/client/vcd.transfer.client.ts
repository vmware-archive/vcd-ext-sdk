...
import {catchError, expand, flatMap, map, retry, switchMap} from "rxjs/operators";

/**
 * Default chunk size is 50MiB.  This is equal to the UI itself.
 */
 ...
 */
export const CHUNK_RETRY_COUNT = 5;
...
export class VcdTransferClient {
     ...
     */
    constructor(public httpClient: HttpClient, public transferUrl: string) {
    }
    ...
    upload(source: Blob|File, progressObserver?: Observer<TransferProgress>): Observable<TransferResult> {
        ...
        const {httpClient, transferUrl} = this;
        ...
        function createTransferProgress(retryNumber: number, retryCount: number, chunksSent: number): TransferProgress {
            const chunksRemaining = totalChunks - chunksSent;
            const bytesSent = Math.min(chunksSent * CHUNK_SIZE, totalBytes);
            const bytesRemaining = totalBytes - bytesSent;
            const percent = bytesRemaining / totalBytes * 100;
            const timeTakenMs = new Date().getTime() - startTimeMs;
            const estimatedTotalTimeMs = (bytesSent / bytesRemaining) * timeTakenMs;
            const estimatedTimeRemainingMs = estimatedTotalTimeMs - timeTakenMs;
            return {
                filename,  transferUrl, retryNumber, retryCount,
        ...
        function finishUpload(chunksSent: number): TransferResult {
            const transferProgress = createTransferProgress(0, retryCount, chunksSent);
            if (progressObserver) {
        ...
        // Upload all of the chunks and finish the transfer
        return Observable.of(0).pipe(
            // Transfer all chunks in order
            expand((chunkIndex) =>
                transferChunk(chunkIndex).pipe(
                    flatMap(() => {
                        chunkIndex++;
                        return chunkIndex < totalChunks ? Observable.of(chunkIndex) : Observable.empty()
                    })
                )),
