...
@Injectable()
export class ChangeTenantScopeService extends RequestTracker<ChangeScopeRequest> {
    /**
     * Setting the status of the request which be made.
     * @param url the url of the request
     * @param value the status of the request
     */
    public handleCompletedRequest(res: HttpResponse<EntityReference2[]> | HttpErrorResponse): Promise<void> {
            ...
            if (index === -1) {
                ...
                */
                ...
                if (urlCheck[0].length !== 0 && !urlCheck[1]) {
                    reject(new Error("This element does not exist!"));
                    return;
                }
                // Immutable copy
    ...
    public executeRequestsInParallel(): Observable<HttpResponse<EntityReference2[]>> {
